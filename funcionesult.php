...
function GetAcumuladoEvaluacionDocenteCarrera (&$nro,$cedula,$carrera,$lapso,$sede){
    
    $select = "DISTINCT sce090d_nom_asign,eval001d_cod_seccion,eval001d_ponderacion,eval002d_id_evaluacion";
    $from = "sce2000.eval002d,sce2000.eval001d,sce2000.sce090d";
    $where = "eval002d_id_evaluacion = eval001d_id_evaluacion AND sce090d_cod_asign = CAST(eval001d_cod_asignatura AS VARCHAR) AND eval001d_cod_sede = '" . $sede . "' AND eval001d_cod_carrera = '" . $carrera . "' AND eval001d_cedula_docente = '" . $cedula . "' AND eval001d_lapso_academico = '" . $lapso . "'";
...
function GetAcumuladoEvaluacionDocenteAsignatura (&$nro,$cedula,$asignatura,$seccion,$sede,$lapso){
    
    $select = "DISTINCT eval001d_id_evaluacion,eval001d_ponderacion";
    $from = "sce2000.eval001d,sce2000.eval002d";
    $where = "eval002d_id_evaluacion = eval001d_id_evaluacion AND eval001d_cod_sede = '" . $sede . "' AND eval001d_cod_seccion = '" . $seccion . "' AND eval001d_cod_asignatura = '" . $asignatura . "' AND eval001d_cedula_docente = '" . $cedula . "' AND eval001d_lapso_academico = '" . $lapso . "'";
...
function GetAcumuladoEvaluacionDocenteDetallado (&$nro,$cedula,$lapso,$sede){
    
    $select = "DISTINCT sce090d_nom_asign,eval001d_cod_seccion,eval001d_ponderacion,eval001d_cod_carrera,eval001d_cod_sede,eval002d_id_evaluacion";
    $from = "sce2000.eval001d,sce2000.eval002d,sce2000.sce090d,sce2000.eval002d";
    $where = "sce090d_cod_asign = CAST(eval001d_cod_asignatura AS VARCHAR) AND sce090d_cod_asign = CAST(eval001d_cod_asignatura AS VARCHAR) AND eval002d_id_evaluacion = eval001d_id_evaluacion AND eval001d_cod_sede = '" . $sede . "'" . " AND eval001d_cedula_docente = '" . $cedula . "' AND eval001d_lapso_academico = '" . $lapso . "'";

    $groupby = "";
    $orderby = "eval001d_cod_carrera,eval001d_cod_sede,sce090d_nom_asign,eval001d_cod_seccion"; 
...
function ConsultarProductosActa(&$nro,$lapsoacademico,$sede,$carrera,$seccion,$asignatura,$lapsovigencia,$idproductosdef){
    ...
    $select = "DISTINCT eval002d_id_evaluacion,eval001d_producto,eval001d_sem_planificacion";
...
$select = "eval002d_cedula_est,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,eval002d_nota,CAST(eval002d_no_asistio AS INT) ". 
          "FROM( ".
          "select DISTINCT eval002d_cedula_est,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,eval002d_nota,eval002d_no_asistio";
$from = "sce2000.sce049d,sce2000.eval002d";
$where = "eval002d_cedula_est = cedula ".
         ...
         "select sce020d_cedula AS eval002d_cedula_est,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre, 0 AS eval002d_nota,0 eval002d_no_asistio " .
...
function GuardarPonderacion($msg,$ideval,$cedula,$expediente,$carrera,$asignatura,$sede,$seccion,$lapsoacademico,$lapsovigencia,$poderacion,$definitiva,$abandono,$asistio,$cedulaDocente,$nroacta,$semana,$fecha_proceso,$fecha_act,$cedulaDocenteact,$estatus,$fechaevaluacion,$justificacion){
    ...
    eval002d_responsable,eval002d_nroacta,eval002d_sem_aplicacion,eval002d_fecha_registro,eval002d_fecha_ua,eval002d_resp_ua,eval002d_estatus,eval002d_fechaevaluacion,eval002d_justificacion";
    ...
    $s_valores= $s_valores . "" . $ideval . ",'" . $cedula . "'," .$expediente. ",'" .$carrera. "'," .$asignatura. ",'" . 
            ...
            $semana. "','" .$fecha_proceso. "','" .$fecha_act . "','" . $cedulaDocenteact . "','" . $estatus . "','" . $fechaevaluacion . "','" . $justificacion;
...
function GuardarEvaluacion(&$msg,$id_evaluacion,$sede,$carrera,$seccion,$asignatura,$lapsoacademico,$lapsovigencia,$cedulaDocente,$producto,$actividad,$criterio,$instrumento,$evaluacion,$fecha_proceso,$status,$semana,$fecha_proceso,$cedulaDocente,$ideval){
  ...
  if($ideval == 0){
    ...
    $s_valores=$s_valores . "'" .$producto. "','" .$actividad. "','" .$criterio. "'," .$instrumento. "," .$evaluacion. ",'" .$semana. "','" .$fecha_proceso. "','" .$cedulaDocente . "'";
...
function Consultar_Asig_Lapso_Docente(&$nro,$lapso,$docente){
   ...
   "select DISTINCT ' ' AS sce080d_nom_carr,sce090d_nom_asign,sce070d_seccion,sce070d_lapso_vigencia,'0' as cod_carr,sce070d_cod_asign,sce070d_sede,replace(sce016d_alias_largo,'-','') as lapso,sce070d_cedula_doc,sce025d_descripcion,nombre,sce110d_semestre,sce070d_lapso as lapso_sys " .
