...
$(document).ready(function () {
    ...
    //APROBAR LA ULTIMA EVALUACION
     $("#bot_reiniciarevaluacion").click(function(){
        ...
        alertify.confirm("SE BORRARÁN TODAS LAS NOTAS REGISTRADAS. DESEA CONTINUAR?", function (e) {
            if (e){
                ...
                $.ajax({
                    ...
                    data:{Cedula:Cedula,Carrera:cod_carr,Asignatura:cod_asign,Seccion:seccion,Sede:cod_sede,LapsoAcademico:lapsoacademico,LapsoVigencia:lapsovigencia,Opcion:15}
    ...
    //APROBAR LA ULTIMA EVALUACION
     $("#bot_ok").click(function(){
        ...
        alertify.confirm("Desea Aprobar la última evaluación?", function (e) {
            if (e){
                $.ajax({
                   ...
                   if (data == 1){ 
                        alertify.success("Evaluación Aprobada Satisfactoriamente");    
                        
                        $('.GenerarProductos').click();
                   }      
                   else{
                       alertify.error("Error en Aprobar la Evaluación");    
                   }
                });  
            }
            else {
    ...
    $("#bot_buscarlapso").click(function(){
       ...
       if (valor == "LAPSO"){
           ...
           if ($("#cmb_lapsoconsulta").val() != 0){
            ...
            alertify.alert("SELECCIONE EL LAPSO ACADÉMICO A CONSULTAR");
           }
    ...
    function Habilitado(){
         ...
         $.ajax({
            ...
            if (data == 'S'){
                win = window.open('eval999.php' , '_self');
                if (win) {
                ...
                if (semevaluacion != "0") {
                    ...
                    //if (validarfechaevaluacion(fechaevaluacion)){
                       ...
                       if (apto){
                            ...
                            $("#preloader").css("display", "none");
                            alertify.alert("NO SE ACTUALIZÓ. HAY PONDERACIONES MAYORES A " + ponderacion + "%");
                    ...
                    $("#preloader").css("display", "none");            
                    alertify.alert("SELECCIONE LA SEMANA DE EVALUACIÓN PARA GUARDAR.");
       ...
       if (status == 1){
            ...
            //$("#date_fechaevaluacion").prop("disabled", false);
             ...
             $.ajax({
                 ...
                 type: 'POST',
                 url: 'eval006Dao.php?'+ Sesion,
                 data:{Ponderacion:ponderacion,Cedula:Cedula,Seccion:seccion,LapsoVigencia:lapsovigencia,Carrera:codcarr,Asignatura:codasign,Sede:codsede,LapsoAcademico:lapsoacademico,IdEval:ideval,DesCarrera:descarr,DesAsignatura:desasign,DesSemestre:semestre,DesProducto:producto,Opcion:5}
            ...
            alertify.alert("DEBE EVALUAR EL PRODUCTO "  + productoevaluar + " PONDERACIÓN " + ponderacionevaluar + " SEMANA " + semanaevaluar);
    ...
    $("#table_producto").on('click', '.AgregarEvaluacion', function () {
       ...
       if (status == -1){
            ...
            if (id_evaprim == ideval){
                alertify.alert("PARA ESTA VERSIÓN SE REALIZARON LAS SIGUIENTES ACTUALIZACIONES: 1) LA ESCALA DE LA NOTA ES PORCENTUAL. PARA ESTE PRODUCTO O EVIDENCIA EL REGISTRO DE LA NOTA ES A PARTIR DE 1% (MÍNIMO) AL  "  + ponderacion + "% (MÁXIMO). 2) OPCIÓN DE AGREGAR HASTA DOS DECIMALES EN EL REGISTRO DE NOTAS. 3) SE ACTUALIZÓ LA FECHA DE EVALUACIÓN POR SEMANA DE EVALUACIÓN. 4) OPCIÓN DE DESCARGAR EN FORMATO PDF LA EVALUACIÓN DEL PRODUCTO O EVIDENCIA. 5) OPCIÓN DE VISUALIZAR LA INFORMACIÓN ESTADÍSTICA DE LOS ESTUDIANTES CURSANTES EN LA UNIDAD CURRICULAR. 6) OPCIÓN DE REINICIAR TODO EL REGISTRO DE NOTAS  6) OPCIÓN DE DESCARGAR EL ACTA DE NOTAS DEFINITIVA UNA VEZ QUE SE HAGA EL CIERRE DE NOTAS POR PARTE DE LA COORDINACIÓN DE INFORMÁTICA Y ESTADÍSTICA.");
